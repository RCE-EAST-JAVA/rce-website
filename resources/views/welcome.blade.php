...
@section('content')
    ...
    @php
        $heroImages = $heroPhotos->isNotEmpty()
            ...
            : json_encode([
                'https://images.unsplash.com/photo-1542273917363-3b1817f69a2d?auto=format&fit=crop&q=80&w=1920',
            ]);
    @endphp

    <div class="relative bg-zinc-950 text-white overflow-hidden min-h-screen" x-data="{
        slides: {{ $heroImages }},
        current: 0,
        timer: null,
        scrollY: 0,
        ...
        <div class="absolute inset-0 bg-gradient-to-tr from-zinc-950/80 via-zinc-900/40 to-transparent pointer-events-none">
        </div>
        ...
        <template x-if="slides.length > 1">
            <div class="absolute bottom-6 left-1/2 -translate-x-1/2 z-20 flex gap-2">
                <template x-for="(slide, index) in slides" :key="index">
                    <button @click="goTo(index)" class="rounded-full transition-all duration-300"
        ...
        @if ($heroPhotos->isNotEmpty())
            <div class="absolute bottom-10 right-6 z-20 hidden md:block">
                <template x-for="(slide, index) in slides" :key="'cap-' + index">
                    <p x-show="current === index" x-transition:enter="transition ease-out duration-500"
                ...
                @foreach ($heroPhotos as $i => $photo)
                    @if ($photo->caption)
                        <p x-show="current === {{ $i }}" x-transition:enter="transition ease-out duration-500"
                            x-transition:enter-start="opacity-0 translate-y-2"
                            x-transition:enter-end="opacity-100 translate-y-0"
                            class="text-xs text-white/50 italic text-right max-w-xs">
                            {{ $photo->caption }}
        ...
        <div class="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-40 pb-32 md:pt-52 md:pb-48 flex flex-col justify-center min-h-screen"
            ...
            <p data-aos="fade-up" data-aos-delay="100"
                class="text-lg md:text-xl text-gray-300 max-w-2xl mb-10 leading-relaxed">
                Connecting education, research, and local initiatives to accelerate the achievement of the
                Sustainable Development Goals (SDGs) across East Java.
            </p>

            <div data-aos="fade-up" data-aos-delay="150" class="flex flex-wrap gap-4 mb-16">
                <a href="{{ route('projects.index') }}"
                    class="bg-accent-orange hover-bg-accent-orange text-white px-8 py-4 rounded-full font-bold shadow-lg transition duration-200">
                    Explore Programs
                </a>
                <a href="#contact"
                    class="bg-zinc-800/80 hover:bg-zinc-700 text-white border border-zinc-700 px-8 py-4 rounded-full font-bold backdrop-blur transition duration-200">
                    Contact Us
                </a>
            </div>
            ...
            <div data-aos="fade-up" data-aos-delay="200"
                class="grid grid-cols-3 gap-6 max-w-xl border-t border-zinc-800/80 pt-10">
                <div>
                    <span class="block text-3xl md:text-4xl font-extrabold text-white">{{ $stats['projects'] }}+</span>
                    <span
                        class="text-xs md:text-sm text-gray-500 uppercase tracking-widest font-semibold mt-1 block">Program
                        Initiatives</span>
                </div>
                <div>
                    <span class="block text-3xl md:text-4xl font-extrabold text-white">{{ $stats['years'] }}</span>
                    <span class="text-xs md:text-sm text-gray-500 uppercase tracking-widest font-semibold mt-1 block">Years
                        of Impact</span>
                </div>
                <div>
                    <span class="block text-3xl md:text-4xl font-extrabold text-white">{{ $stats['staff'] }}+</span>
                    <span
                        class="text-xs md:text-sm text-gray-500 uppercase tracking-widest font-semibold mt-1 block">Academics
                        &amp; Researchers</span>
                </div>
            </div>
        </div>
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-12 items-stretch">
                <div data-aos="fade-up"
                    class="bg-gradient-to-br from-emerald-950 to-emerald-900 text-white p-10 rounded-3xl flex flex-col justify-between shadow-xl">
                    <div>
                        <span class="text-xs font-bold uppercase tracking-widest text-emerald-400 mb-2 block">Vision</span>
                        <h2 class="text-3xl font-bold leading-snug mb-6">A leading regional centre of excellence for
                            sustainable development in East Java.</h2>
                ...
                <div data-aos="fade-up" data-aos-delay="50"
                    class="bg-amber-600 text-white p-10 rounded-3xl flex flex-col justify-between shadow-xl">
                    <div>
                    ...
                    <div class="border-t border-amber-500 pt-6">
                        <span class="text-sm text-amber-200">Accredited by UNU-IAS</span>
                    </div>
                </div>

                <div data-aos="fade-up" data-aos-delay="100"
                    class="bg-zinc-50 border border-zinc-100 p-10 rounded-3xl flex flex-col justify-between shadow-md">
                    <div>
                        <span class="text-xs font-bold uppercase tracking-widest text-primary-green mb-2 block">Our
                            Mission</span>
                        <ul class="space-y-4 text-sm text-gray-600">
                            <li class="flex gap-3">
                                <span
                                    class="bg-orange-100 text-accent-orange w-6 h-6 rounded-full flex items-center justify-center font-bold text-xs shrink-0">1</span>
                                <span>Promoting the integration of SDG principles into both formal and
                                    informal education curricula.</span>
                            </li>
                            <li class="flex gap-3">
                                <span
                                    class="bg-orange-100 text-accent-orange w-6 h-6 rounded-full flex items-center justify-center font-bold text-xs shrink-0">2</span>
                                <span>Facilitating collaborative research between universities and practitioners to generate
                                    real-world solutions in East Java.</span>
                            </li>
                            <li class="flex gap-3">
                                <span
                                    class="bg-orange-100 text-accent-orange w-6 h-6 rounded-full flex items-center justify-center font-bold text-xs shrink-0">3</span>
                                <span>Building awareness and capacity of local communities for environmental
                                    action.</span>
                            </li>
                        </ul>
                    </div>
    ...
    <div class="py-24 bg-white">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div data-aos="fade-up" class="flex justify-between items-end mb-12">
                ...
                <a href="{{ route('projects.index') }}"
                    class="text-sm font-bold text-accent-orange hover:text-amber-700 flex items-center gap-1">
                    ...
                    <a data-aos="fade-up" href="{{ route('projects.show', $featured->slug) }}"
                        ...
                        <div class="relative h-72 bg-zinc-200 overflow-hidden">
                            @if ($featured->display_image)
                                <img src="{{ asset($featured->display_image) }}"
                                    class="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500">
                            @else
                                <div
                                    class="w-full h-full flex items-center justify-center bg-emerald-950/10 text-primary-green">
                                    <i class="bi bi-image text-5xl"></i>
                                </div>
                            @endif
                            <span
                                class="absolute top-4 left-4 bg-accent-orange text-white text-xs px-3 py-1.5 rounded-full font-bold shadow-md">
                                {{ $featured->category }}
                            </span>
                        </div>
                        <div class="p-7 flex flex-col flex-1">
                            ...
                            <h3
                                class="font-extrabold text-xl text-gray-900 mb-3 line-clamp-2 group-hover:text-accent-orange transition-colors">
                                {{ $featured->title }}</h3>
                                ...
                                @if ($featured->sdgs)
                                    <div class="flex gap-1">
                                        @foreach (explode(',', $featured->sdgs) as $sdg)
                                            <span
                                                class="text-[10px] bg-accent-orange/10 text-accent-orange font-bold px-2 py-0.5 rounded">{{ trim($sdg) }}</span>
                                        @endforeach
                                    </div>
                                @endif
                            </div>
                            ...
                            <a href="{{ route('projects.show', $project->slug) }}"
                                ...
                                <div class="w-28 flex-shrink-0 rounded-2xl overflow-hidden bg-zinc-200 self-stretch">
                                    @if ($project->display_image)
                                        <img src="{{ asset($project->display_image) }}"
                                            class="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500">
                                    @else
                                        <div
                                            class="w-full h-full flex items-center justify-center bg-emerald-950/10 text-primary-green">
                                ...
                                <div class="flex flex-col justify-center min-w-0">
                                    <span
                                        class="text-[10px] font-bold uppercase tracking-widest text-accent-orange mb-1">{{ $project->category }}</span>
                                    <h3
                                        class="font-bold text-sm text-gray-900 line-clamp-2 group-hover:text-accent-orange transition-colors leading-snug mb-1">
                                        {{ $project->title }}</h3>
        ...
        <div class="py-24 bg-zinc-50 border-t border-zinc-100">
            <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                <div data-aos="fade-up" class="flex items-end justify-between mb-10">
                    <div>
                        <span class="text-xs font-bold uppercase tracking-widest text-accent-orange mb-2 block">From Our
                            Blog</span>
                ...
                @php
                    ...
                    $restArticles = $latestArticles->skip(1);
                @endphp
                ...
                <a data-aos="fade-up" href="{{ route('articles.show', $featuredArticle->slug) }}"
                    ...
                    <div class="md:w-2/5 aspect-[16/9] md:aspect-auto bg-zinc-100 overflow-hidden flex-shrink-0">
                        ...
                        @else
                            <div
                                class="w-full h-full min-h-[200px] flex items-center justify-center bg-gradient-to-br from-emerald-50 to-emerald-100">
                                <svg xmlns="http://www.w3.org/2000/svg" class="w-16 h-16 text-emerald-200" fill="none"
                    ...
                    <div class="flex flex-col justify-center p-8 flex-1">
                        <div class="flex items-center gap-2 mb-3">
                            <span
                                class="text-xs font-semibold bg-orange-50 text-orange-700 px-2.5 py-1 rounded-full">{{ $featuredArticle->category }}</span>
                            <span
                                class="text-xs text-zinc-400">{{ $featuredArticle->published_at?->format('d M Y') }}</span>
                        </div>
                        <h3
                            class="font-extrabold text-2xl text-zinc-900 leading-snug mb-3 group-hover:text-accent-orange transition line-clamp-2">
                            {{ $featuredArticle->title }}
                        </h3>
                        @if ($featuredArticle->excerpt)
                            <p class="text-zinc-500 text-sm leading-relaxed line-clamp-3 mb-4">
                ...
                @if ($restArticles->isNotEmpty())
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        @foreach ($restArticles as $i => $article)
                            <a data-aos="fade-up" data-aos-delay="{{ $i * 50 }}"
                                ...
                                <div class="w-28 h-28 flex-shrink-0 rounded-xl overflow-hidden bg-zinc-100">
                                    ...
                                    @else
                                        <div
                                            class="w-full h-full flex items-center justify-center bg-gradient-to-br from-emerald-50 to-emerald-100">
                                            <svg xmlns="http://www.w3.org/2000/svg" class="w-8 h-8 text-emerald-200"
                                ...
                                <div class="flex flex-col justify-center min-w-0">
                                    <div class="flex items-center gap-2 mb-1.5">
                                        <span
                                            class="text-[10px] font-bold uppercase tracking-widest text-accent-orange">{{ $article->category }}</span>
                                        <span
                                            class="text-[10px] text-zinc-400">{{ $article->published_at?->format('d M Y') }}</span>
                                    </div>
                                    <h3
                                        class="font-bold text-sm text-zinc-900 line-clamp-2 group-hover:text-accent-orange transition leading-snug mb-1.5">
                                        {{ $article->title }}
                                    </h3>
                                    @if ($article->excerpt)
                                        <p class="text-xs text-zinc-500 line-clamp-2 leading-relaxed mb-1.5">
                                            {{ $article->excerpt }}</p>
                                    @endif
    ...
    <!-- CTA Banner -->
    <div data-aos="fade-up" class="py-24 bg-zinc-100" id="contact">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="bg-zinc-950 p-12 md:p-16 rounded-[2.5rem] shadow-xl text-center relative overflow-hidden">
                <!-- Gradient overlay -->
                <div
                    class="absolute inset-0 bg-gradient-to-br from-emerald-950/60 via-zinc-950 to-zinc-950 pointer-events-none">
                </div>
                <!-- Background image -->
                <div class="absolute inset-0 bg-cover bg-center opacity-10"
                    style="background-image: url('https://images.unsplash.com/photo-1448375240586-882707db888b?auto=format&fit=crop&q=80&w=1200');">
                </div>
                <!-- Decorative blob -->
                <div class="absolute top-0 right-0 w-72 h-72 rounded-full bg-emerald-500/5 blur-3xl pointer-events-none">
                </div>
                <!-- Orange accent line -->
                ...
                <div class="relative z-10 max-w-2xl mx-auto">
                    ...
                    <div class="flex justify-center gap-4 flex-wrap">
                        <a href="mailto:info@rce-eastjava.org"
                            class="bg-accent-orange hover-bg-accent-orange text-white px-8 py-3.5 rounded-full font-bold shadow-lg transition">
